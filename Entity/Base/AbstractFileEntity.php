...
abstract class AbstractFileEntity extends EntityAccess
{
    ...
    /**
     * @ORM\Column(name="myFile", length=255)
     * @Assert\NotBlank()
     * @Assert\Length(min="0", max="255")
     * @var string $myFileFileName
     */
    protected $myFileFileName = null;
     ...
     * @Assert\NotBlank()
     * @Assert\Date()
     * @Assert\GreaterThan(propertyPath="startDate", message="The start must be before the end.")
     * @var date $endDate
     */
    protected $endDate;
    ...
    public function getMyFile()
    {
        ...
        $fileName = $this->myFileFileName;
        if (!empty($fileName) && !$this->_uploadBasePath) {
            throw new \RuntimeException('Invalid upload base path in ' . get_class($this) . '#getMyFile().');
        }
    
        $filePath = $this->_uploadBasePath . 'myfile/' . $fileName;
        if (!empty($fileName) && file_exists($filePath)) {
            $this->myFile = new File($filePath);
            $this->setMyFileUrl($this->_uploadBaseUrl . '/' . $filePath);
        } else {
            $this->setMyFileFileName('');
            $this->setMyFileUrl('');
            $this->setMyFileMeta([]);
        }
    ...
    {
        if (null === $this->myFile && null === $myFile) {
            return;
        }
        if (null !== $this->myFile && null !== $myFile && $this->myFile->getRealPath() === $myFile->getRealPath()) {
            return;
        }
        $this->myFile = isset($myFile) ? $myFile : '';
    
        if (null === $this->myFile || '' == $this->myFile) {
            $this->setMyFileFileName('');
            $this->setMyFileUrl('');
            $this->setMyFileMeta([]);
        } else {
            $this->setMyFileFileName($this->myFile->getFilename());
        }
    }
    
    /**
     * Returns the my file file name.
     *
     * @return string
     */
    public function getMyFileFileName()
    {
        return $this->myFileFileName;
    }
    
    /**
     * Sets the my file file name.
     *
     * @param string $myFileFileName
     *
     * @return void
     */
    public function setMyFileFileName($myFileFileName)
    {
        if ($this->myFileFileName !== $myFileFileName) {
            $this->myFileFileName = isset($myFileFileName) ? $myFileFileName : '';
        }
    }
