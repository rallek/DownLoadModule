...
abstract class AbstractFileController extends AbstractController
{
    ...
    protected function indexInternal(Request $request, $isAdmin = false)
    {
        ...
        $permissionHelper = $this->get('rk_download_module.permission_helper');
    ...
    protected function viewInternal(Request $request, $sort, $sortdir, $pos, $num, $isAdmin = false)
    {
        ...
        foreach ($templateParameters['items'] as $file) {
            if (!$permissionHelper->hasEntityPermission($file, $permLevel)) {
                continue;
            }
     ...
     *
     * @param Request $request Current request instance
     * @param integer $id Identifier of treated file instance
     *
     * @return Response Output
     *
     * @throws AccessDeniedException Thrown if the user doesn't have required permissions
     * @throws NotFoundHttpException Thrown if file to be displayed isn't found
     */
    public function adminDisplayAction(Request $request, $id)
    {
        $file = $this->get('rk_download_module.entity_factory')->getRepository('file')->selectById($id);
        if (null === $file) {
            throw new NotFoundHttpException($this->__('No such file found.'));
        }
    ...
    protected function displayInternal(Request $request, FileEntity $file, $isAdmin = false)
    {
        ...
        $controllerHelper = $this->get('rk_download_module.controller_helper');
        $templateParameters = $controllerHelper->processDisplayActionParameters($objectType, $templateParameters, $file->supportsHookSubscribers());
     ...
     *
     * @throws AccessDeniedException Thrown if the user doesn't have required permissions
     * @throws NotFoundHttpException Thrown if file to be deleted isn't found
     * @throws RuntimeException      Thrown if another critical error occurs (e.g. workflow actions not available)
     */
    ...
    protected function deleteInternal(Request $request, $id, $isAdmin = false)
    {
        ...
        if (!$permissionHelper->hasEntityPermission($file, $permLevel)) {
            ...
            if (!$isOwner || !$permissionHelper->mayEdit($file)) {
                throw new AccessDeniedException();
            }
        }
        ...
        $form = $this->createForm(DeletionType::class, $file);
