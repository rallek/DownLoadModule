...
abstract class AbstractControllerHelper
{
    ...
    public function processViewActionParameters($objectType, SortableColumns $sortableColumns, array $templateParameters = [], $hasHookSubscriber = false)
    {
        ...
        $isAdminArea = false !== strpos($routeName, 'rkdownloadmodule_' . strtolower($objectType) . '_admin');
        if (!$isAdminArea && in_array($objectType, ['file'])) {
            $showOnlyOwnEntries = (bool)$this->variableApi->get('RKDownLoadModule', $objectType . 'PrivateMode', false);
            if (true == $showOnlyOwnEntries) {
                $templateParameters['own'] = 1;
            } else {
                $templateParameters['own'] = (bool)$request->query->getInt('own', $this->variableApi->get('RKDownLoadModule', 'showOnlyOwnEntries', false)) ? 1 : 0;
            }
        } else {
            $templateParameters['own'] = (bool)$request->query->getInt('own', $this->variableApi->get('RKDownLoadModule', 'showOnlyOwnEntries', false)) ? 1 : 0;
        }
        ...
        if (true === $hasHookSubscriber) {
            ...
            $templateParameters['currentUrlObject'] = new RouteUrl('rkdownloadmodule_' . strtolower($objectType) . '_view', $urlParameters);
        }
    ...
    public function processDisplayActionParameters($objectType, array $templateParameters = [], $hasHookSubscriber = false)
    {
        ...
        if (true === $hasHookSubscriber) {
            ...
            $templateParameters['currentUrlObject'] = new RouteUrl('rkdownloadmodule_' . strtolower($objectType) . '_display', $urlParameters);
        }
